...
const FollowRelatedCard = ({ person, handleShowProfile }) => {
  const id = person.handle;
    ...
    <Link to={`/${id}`}>
      ...
      {/* <div onClick={() => handleShowProfile(id)} className="friendListCard"> */}
        <div className="flex items-center justify-between gap-3 ml-2">
          ...
          <img className="w-16 h-16 rounded-full mx-2" src={person.photo} alt={person.name} />
          <div className="text-black dark:text-dark-primary-200">
            <h3 className="text-md font-bold mb-0">@{person.handle}</h3>
            <h5 className="text-md font-bold opacity-75">{person.name}</h5>
          </div>
        </div>
      </div>
    </Link>
...
export default FollowRelatedCard;
