    ...
    <>
      {expandMealType ? (
        <div
          ...
          className={`flex items-center justify-center w-full h-24 mb-1 postCard ${filteredPosts.length === 1 ? 'mb-20' : ''}`}
          style={{
        ...
        mealTypes?.map((mealType, index) => {
          if (mealNames.includes(mealType)) {
            ...
            return (
              <div
                key={mealType}
                className={`flex items-center justify-center w-full h-24 mb-1 postCard ${index === mealTypes.length - 1 ? 'mb-20' : ''}`}
                style={{
